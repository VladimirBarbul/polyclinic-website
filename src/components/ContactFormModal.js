...
const ContactFormModal = memo(({ isOpen, onClose, doctorName }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const messageWithDoctor = doctorName 
        ? `Запис на прийом до ${doctorName}\n\n${formData.message}`
        : formData.message;
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-800">
              Запис на прийом{doctorName ? ` до ${doctorName}` : ''}
            </h2>
            <button
              onClick={onClose}
